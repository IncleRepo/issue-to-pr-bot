"""High-level runtime orchestration for issue, PR, and review events."""
...
def load_event_payload() -> dict:
    ...
    if not event_path:
        ...
        return {
            "action": "created",
            "comment": {
                "body": "@incle-issue-to-pr-bot README local run section 추가해줘",
                "id": 1,
...
def run_bot(workspace: Path, config: BotConfig, request: IssueRequest) -> None:
    ...
    if runtime_options.request_merge:
        merge_result = handle_merge_request(request, result.pull_request_url)
...
def handle_merge_request(request: IssueRequest, pull_request_url: str | None = None) -> MergeRequestResult:
