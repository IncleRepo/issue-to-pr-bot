"""중앙 매니저 설치와 운영 흐름을 담당하는 서비스 모듈.

제어면 Worker, 로컬 agent, 대상 저장소 초기화를 하나의 CLI 경험으로 묶는다.
사용자 입력 파싱은 단순하게 유지하고, 실제 부트스트랩 동작은 이 모듈에서 처리한다.
"""
...
from app.config import BOT_MENTION
from app.release_channel import install_standalone_binary
from app.runtime.comments import configure_output_encoding
from app.versioning import RELEASE_REPOSITORY
...
AUTO_INSTALL_PACKAGES = {
    ...
    "npm": {"label": "Node.js", "winget": "OpenJS.NodeJS.LTS", "choco": "nodejs-lts", "apt": "npm"},
}
...
class AgentBootstrapOptions:
    ...
    workspace_root: Path
    install_root: Path = DEFAULT_AGENT_INSTALL_ROOT
...
def main(argv: Sequence[str] | None = None) -> int:
    """중앙 매니저 CLI 요청을 실제 부트스트랩 동작으로 라우팅한다."""
...
def build_parser() -> argparse.ArgumentParser:
    """사용자 입력을 일관된 옵션 구조로 변환하는 CLI 파서를 만든다."""
    ...
    control_plane_parser.add_argument("--bot-mention", default=BOT_MENTION, help="댓글에서 사용할 봇 멘션입니다.")
    control_plane_parser.add_argument("--agent-repository", action="append", dest="agent_repositories", default=[], help="중앙에서 agent가 처리하도록 허용할 GitHub 저장소입니다. 여러 번 지정할 수 있습니다.")
    control_plane_parser.add_argument("--agent-poll-interval-seconds", type=int, default=10, help="중앙에서 내려줄 agent polling 주기(초)입니다.")
    control_plane_parser.add_argument("--agent-max-concurrency", type=int, default=2, help="중앙에서 agent가 동시에 처리할 최대 task 수입니다.")
    ...
    agent_parser.add_argument("--install-root", default=str(DEFAULT_AGENT_INSTALL_ROOT), help="agent 실행 파일을 설치할 경로입니다.")
    ...
    agent_parser.add_argument("--max-concurrency", type=int, default=2, help="로컬 fallback 설정에서 동시에 처리할 최대 task 수입니다.")
    agent_parser.add_argument("--release-repository", default=RELEASE_REPOSITORY, help="agent를 내려받을 GitHub Releases 저장소입니다.")
    agent_parser.add_argument("--skip-task", action="store_true", help="자동 시작 등록을 건너뜁니다.")
    agent_parser.add_argument("--task-name", default="issue-to-pr-bot-agent", help="자동 시작 등록에 사용할 서비스 이름입니다.")
    ...
    bootstrap_all_parser.add_argument("--install-root", default=str(DEFAULT_AGENT_INSTALL_ROOT), help="agent 실행 파일을 설치할 경로입니다.")
    ...
    bootstrap_all_parser.add_argument("--max-concurrency", type=int, default=2, help="로컬 fallback 설정에서 동시에 처리할 최대 task 수입니다.")
    bootstrap_all_parser.add_argument("--release-repository", default=RELEASE_REPOSITORY, help="agent를 내려받을 GitHub Releases 저장소입니다.")
    bootstrap_all_parser.add_argument("--skip-task", action="store_true", help="자동 시작 등록을 건너뜁니다.")
    bootstrap_all_parser.add_argument("--task-name", default="issue-to-pr-bot-agent", help="자동 시작 등록에 사용할 서비스 이름입니다.")
...
def add_control_plane_bootstrap_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--bot-mention", default=BOT_MENTION, help="댓글에서 사용할 봇 멘션입니다.")
    parser.add_argument("--agent-repository", action="append", dest="agent_repositories", default=[], help="중앙에서 agent가 처리하도록 허용할 GitHub 저장소입니다. 여러 번 지정할 수 있습니다.")
    parser.add_argument("--agent-poll-interval-seconds", type=int, default=10, help="중앙에서 내려줄 agent polling 주기(초)입니다.")
    parser.add_argument("--agent-max-concurrency", type=int, default=2, help="중앙에서 agent가 동시에 처리할 최대 task 수입니다.")
...
def build_agent_bootstrap_options(args: argparse.Namespace) -> AgentBootstrapOptions:
    return AgentBootstrapOptions(
        ...
        install_root=Path(getattr(args, "install_root", DEFAULT_AGENT_INSTALL_ROOT)).resolve(),
...
def build_bootstrap_all_options(args: argparse.Namespace) -> BootstrapAllOptions:
    ...
    combined_repositories = list(getattr(args, "repositories", [])) or list(control_plane.agent_repositories)
    if not control_plane.agent_repositories and combined_repositories:
        control_plane = ControlPlaneBootstrapOptions(
            target=control_plane.target,
            worker_name=control_plane.worker_name,
            bot_mention=control_plane.bot_mention,
            github_app_id=control_plane.github_app_id,
            github_app_private_key_file=control_plane.github_app_private_key_file,
            agent_repositories=combined_repositories,
            agent_poll_interval_seconds=control_plane.agent_poll_interval_seconds,
            agent_max_concurrency=control_plane.agent_max_concurrency,
            agent_token=control_plane.agent_token,
            webhook_secret=control_plane.webhook_secret,
            force=control_plane.force,
            dry_run=control_plane.dry_run,
        )
    agent = AgentBootstrapOptions(
...
def init_target_repository(options: TargetRepositoryOptions) -> InstallManagerResult:
    ...
    return InstallManagerResult(
        ...
        next_steps=[
            "GitHub App을 대상 저장소에 설치하세요.",
            "Cloudflare Worker webhook URL을 GitHub App 설정에 연결하세요.",
            "아직 agent를 만들지 않았다면 `bootstrap-agent`를 먼저 실행하세요.",
            "필요하면 `AGENTS.md`와 `.issue-to-pr-bot.yml`을 프로젝트 규칙에 맞게 수정하세요.",
        ],
    )
...
def init_control_plane_environment(options: ControlPlaneOptions) -> InstallManagerResult:
    ...
    replacements = {
        ...
        "{{AGENT_REPOSITORIES}}": ",".join(options.agent_repositories),
        "{{AGENT_POLL_INTERVAL_SECONDS}}": str(options.agent_poll_interval_seconds),
        "{{AGENT_MAX_CONCURRENCY}}": str(options.agent_max_concurrency),
    }
    specs = [
    ...
    return InstallManagerResult(
        ...
        operations=operations,
        next_steps=[
            "다음 단계로 `bootstrap-control-plane`을 실행하세요.",
            "필요하면 Worker 프로젝트에서 `npm install`을 먼저 실행하세요.",
            "Cloudflare KV namespace가 생성되면 `wrangler.jsonc`의 `TASK_QUEUE` id가 채워집니다.",
            "필요한 secret은 `npx wrangler secret put ...`으로 등록합니다.",
            "마지막으로 `npx wrangler deploy`로 배포합니다.",
        ],
    )


def bootstrap_control_plane_environment(options: ControlPlaneBootstrapOptions) -> ControlPlaneBootstrapResult:
    """Worker 제어면을 생성하고 secret 등록과 배포까지 한 번에 수행한다."""
    ...
    install_result = init_control_plane_environment(
        ControlPlaneOptions(
            ...
            worker_name=options.worker_name,
            bot_mention=options.bot_mention,
            agent_repositories=options.agent_repositories,
            agent_poll_interval_seconds=options.agent_poll_interval_seconds,
            agent_max_concurrency=options.agent_max_concurrency,
            force=options.force,
            dry_run=options.dry_run,
        )
    )
...
def bootstrap_agent_environment(options: AgentBootstrapOptions) -> AgentBootstrapResult:
    """로컬 agent 설정 파일과 실행 보조 자원을 준비한다."""
    runtime_path = options.install_root / resolve_agent_binary_name()
    runtime_version: str | None = None
    operations: list[FileOperation] = []
    if not options.dry_run:
        ensure_command_available("git")
        ensure_command_available("codex")
        codex_auth = check_codex_auth()
        if codex_auth.status != "pass":
            raise RuntimeError("Codex 로그인이 필요합니다. `codex login` 후 다시 실행하세요.")
        ...
        runtime_path, runtime_action, runtime_version = install_standalone_binary(
            "agent",
            options.install_root,
            repository=options.release_repository,
        )
        operations.append(FileOperation(path=runtime_path, action=runtime_action))
    else:
        operations.append(FileOperation(path=runtime_path, action="would_create"))
    ...
    if runtime_version:
        config_payload["managed_runtime_version"] = runtime_version
    if options.repositories:
        config_payload["repositories"] = options.repositories
    ...
    operations.append(FileOperation(path=options.config_path, action=action))
    task_status = register_agent_scheduled_task(options, runtime_path=runtime_path)
    next_steps = [
        f"agent 경로: `{runtime_path}`",
        "Codex 로그인이 되어 있는지 확인하세요.",
        f"로그 위치: `{options.log_path}`",
    ]
    if options.install_task:
        next_steps.extend(build_agent_autostart_steps(options, task_status, runtime_path=runtime_path))
    else:
        next_steps.append(f"`{runtime_path}` 또는 `issue-to-pr-bot-agent --config {options.config_path}`로 agent를 실행하세요.")
    next_steps.append("실행 후 콘솔에서 `status`, `ps`, `logs latest -f` 같은 내부 명령으로 상태를 확인하세요.")
...
def bootstrap_all_environment(options: BootstrapAllOptions) -> tuple[ControlPlaneBootstrapResult, AgentBootstrapResult, InstallManagerResult]:
    ...
    agent_options = AgentBootstrapOptions(
        ...
        install_root=options.agent.install_root,
        ...
        max_concurrency=options.agent.max_concurrency,
        release_repository=options.agent.release_repository,
...
def check_agent_config(path: Path) -> DoctorCheck:
    ...
    required = {"control_plane_url", "agent_token", "workspace_root"}
...
def resolve_agent_entrypoint() -> Path | None:
    executable_dir = Path(sys.executable).resolve().parent
    for candidate_name in ("issue-to-pr-bot-agent.exe", "issue-to-pr-bot-agent"):
        candidate = executable_dir / candidate_name
        if candidate.exists():
            return candidate
    discovered = shutil.which("issue-to-pr-bot-agent")
    return Path(discovered) if discovered else None


def resolve_agent_binary_name() -> str:
    return "issue-to-pr-bot-agent.exe" if is_windows_platform() else "issue-to-pr-bot-agent"
...
def build_agent_launch_command(config_path: Path, runtime_path: Path | None = None) -> str:
    entrypoint = runtime_path if runtime_path is not None else resolve_agent_entrypoint()
    if entrypoint is not None:
        return f'"{entrypoint}" --config "{config_path}"'
    ...
    return f'"{interpreter}" -m app.agent_runner --config "{config_path}"'


def register_agent_scheduled_task(options: AgentBootstrapOptions, *, runtime_path: Path | None = None) -> str:
    ...
    if is_windows_platform():
        command = build_agent_launch_command(options.config_path, runtime_path)
        try:
            run_checked_command(
                [
                    "schtasks",
    ...
    if is_posix_platform():
        ...
        service_path = service_dir / f"{options.task_name}.service"
        service_body = build_agent_systemd_service(options, runtime_path=runtime_path)
        try:
            service_dir.mkdir(parents=True, exist_ok=True)
            service_path.write_text(service_body, encoding="utf-8")
            run_checked_command(["systemctl", "--user", "daemon-reload"], cwd=options.config_path.parent)
            run_checked_command(["systemctl", "--user", "enable", options.task_name], cwd=options.config_path.parent)
        except RuntimeError as exc:
            return f"failed: {exc}"
        return "created"
...
def build_agent_systemd_service(options: AgentBootstrapOptions, *, runtime_path: Path | None = None) -> str:
    command = build_agent_launch_command(options.config_path, runtime_path)
    return "\n".join(
        [
            "[Unit]",
            "Description=issue-to-pr-bot local agent",
            "After=network-online.target",
            "",
            "[Service]",
            "Type=simple",
            f"WorkingDirectory={options.config_path.parent}",
            f"ExecStart={command}",
            "Restart=on-failure",
            "RestartSec=5",
            "",
...
def build_agent_autostart_steps(options: AgentBootstrapOptions, task_status: str, *, runtime_path: Path | None = None) -> list[str]:
    if is_windows_platform():
        return [
            f"Windows 작업 스케줄러 등록: `{options.task_name}` ({task_status})",
            f"즉시 실행: `schtasks /Run /TN \"{options.task_name}\"`",
            f"직접 실행: `{runtime_path or options.install_root / resolve_agent_binary_name()}`",
        ]
    if is_posix_platform():
        return [
            f"systemd 사용자 서비스 등록: `{options.task_name}` ({task_status})",
            f"즉시 실행: `systemctl --user start {options.task_name}`",
            f"상태 확인: `systemctl --user status {options.task_name}`",
            f"직접 실행: `{runtime_path or options.install_root / resolve_agent_binary_name()}`",
        ]
    return [f"자동 시작 등록: `{options.task_name}` ({task_status})"]
...
def auto_install_command_if_missing(executable: str) -> bool:
    ...
    if manager == "winget":
        ...
        command = [*prefix, "apt-get", "install", "-y", package["apt"]]
    else:
        return False
...
def format_agent_bootstrap_result(result: AgentBootstrapResult) -> str:
    lines = [
        "## 로컬 agent 설정 결과",
        "",
        f"- agent 실행 파일: `{result.runtime_path}`",
        f"- agent 버전: `{result.runtime_version or 'latest'}`",
        f"- 설정 파일: `{result.config_path}`",
        f"- 로그 파일: `{try_extract_log_path(result.config_path) or '설정 파일에서 경로를 읽지 못함'}`",
    ]
    if result.task_name:
        lines.append(f"- 작업 이름: `{result.task_name}`")
    lines.extend(["", "### 변경 파일"])
    if not result.operations:
        lines.append("- 없음")
    else:
