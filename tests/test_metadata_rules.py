...
class MetadataRulesTest(unittest.TestCase):
    ...
    def test_infer_issue_metadata_understands_korean_metadata_sections(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            workspace.joinpath("AGENTS.md").write_text(
                "\n".join(
            ...
            metadata = infer_issue_metadata(workspace, self.make_request("프론트 문구 수정", "문구 수정"))

        self.assertEqual(metadata.assignees, ["@alice"])
        self.assertEqual(metadata.milestone_title, "Sprint 2")

    def test_infer_pull_request_metadata_adds_frontend_label_from_changed_files(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            metadata = infer_pull_request_metadata(
                workspace,
                self.make_request("랜딩 화면 수정", "프론트 화면 다듬기"),
                ["web/index.html", "web/styles.css"],
            )

        self.assertIn("frontend", metadata.pr_labels)

    def test_infer_issue_metadata_does_not_add_bug_label_for_generic_issue_wording(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            workspace = Path(temp_dir)
            metadata = infer_issue_metadata(
                workspace,
                self.make_request("새 전투 화면 구현", "이 이슈 xhigh로 구현부탁해"),
            )

        self.assertNotIn("bug", metadata.issue_labels)
